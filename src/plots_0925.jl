...
function save_rank_panel_pdf(r::Int, resvec::Vector{ALSResult}, 
                             ...
                             names = ["outer=$(resvec[i].outer_tol), inner=$(resvec[i].inner_tol)" for i in range(1, length(resvec))],
                             outfile = "exact")

    K = length(resvec)

    lay = @layout [grid(K, 5); T{0.05h}]
    p = plot(layout=lay, size=(5000, K*700), margin=20mm, left_margin=30mm, 
             plot_title="Exact ALS Rank Panel | matrix size = $(size_B) cond = $cond_B ")

    idx(row, col) = (row-1)*5 + col
    last_ratio = zeros(K)
    ...
    for j in 1:K
        ...
        plot!(p[idx(j,1)], x, y1; yscale=:log10, legend=false,
              xlabel="Outer iteration", title="Error at each outer iteration($(names[j]))")
        ...
        plot!(p[idx(j,2)], x[2:end], ratio; xlabel="Outer iteration",title="Error Ratio ($(names[j]))", legend=false)
        ...
        plot!(p[idx(j,5)], x, y4; yscale=:log10, label="left", 
              xlabel="Outer iteration", title="CG residual matrix in frobenius norm ($(names[j]))")
        plot!(p[idx(j,5)], x, y5; yscale=:log10, label="right")
        
    end

    headers = ["rank","outer_tol", "inner_tol", "final_error", "total_time", "total_inner_iters", "outer_iters", "converged", "final_ratio", "X2tX2_fnorm"]
    rows = [(
        string(resvec[j].rank),
        @sprintf("%.2e", resvec[j].outer_tol),
        @sprintf("%.2e", resvec[j].inner_tol),
...
function save_relaxation_panel_pdf(
    ...
    lay = @layout [grid(7,1)]
    p = plot(layout=lay, size=(3000, 7000), margin=10mm, left_margin=20mm,
             plot_title="ALS Relaxation Panel |matrix size = $(size_B) cond = $cond_B |  rank = $r| outer_tol=$(@sprintf("%.1e", outer_tol)) ")

    vec_inner_tol = [res.inner_tol for res in resvec_inner]
    ...
    for res in resvec_inner
        lab = @sprintf("inner=%.1e", res.inner_tol)
    ...
    headers = ["rank","inner_tol", "outer_tol", "final_err", "total_time", "total_inner_iters","outer_iters", "converged", "X2tX2_fnorm"]
    rows_tbl = [(
        string(res.rank),
        @sprintf("%.2e", res.inner_tol),
