...
# function save_cg_residuals_grid_pdf(res_hist; name="X1 - Krylov")
#     n_outer = length(res_hist)
#     nrows = ceil(Int, sqrt(n_outer))
#     ncols = ceil(Int, n_outer / nrows)

#     plt = plot(layout = (nrows, ncols), size = (300*ncols, 250*nrows))
...
#             res = res_hist[t][j]
#             it = 0:length(res)-1
#             y = copy(res); y[y .<= 0] .= NaN
#             plot!(p, it, y, label = "col $j")
#         end
#         plot!(plt[t], p)  
#     end
#     rankstr = " "
    ...
    for s in 1:outer_maxiters
        ...
        Threads.@threads for core in 1:num_threads
            ...
            for j = (core-1)*blocksize1 +1 : min(core*blocksize1, n) 
                Y1[j, :], ch1_j= cg(A1_cg, @views RHS1_cg[:, j]; atol=inner_abstol, itmax=inner_maxiters, history=true, rtol=inner_reltol)
                
                iters_1[j] = ch1_j.niter
                time_1[j]  =ch1_j.timer
                # Last residual in cg history - check if resnorm is not empty
                if !isempty(ch1_j.residuals)
                    res_col1_j = ch1_j.residuals[end] # true residual vector of the j-th column in 2-norm
                    cg_last_resvec_1[j] = norm(res_col1_j, 2) # true residual norm of the j-th column in 2-norm
                    # save_cg_residuals_grid_pdf(ch1_j.residuals; name="X1_Krylov_cg_all")
                else
        ...
        Threads.@threads for core in 1:num_threads
            ...
            for j = (core-1)*blocksize2 +1 : min(core*blocksize2, m)
                Y2[j, :], ch2_j = cg(A2_cg, @views RHS2_cg[:, j]; atol=inner_abstol, itmax=inner_maxiters, history=true, rtol=inner_reltol)
                
                iters_2[j] = ch2_j.niter
                time_2[j]  =ch2_j.timer
                # Last residual in cg history - check if resnorm is not empty
                if !isempty(ch2_j.residuals)
                    res_col2_j = ch2_j.residuals[end] # true residual vector of the j-th column in 2-norm
                    cg_last_resvec_2[j] = norm(res_col2_j, 2) # true residual norm of the j-th column in 2-norm
                    res_col2_j_all = norm(ch2_j.residuals, 2) # all residuals in history in 2-norm
                    # save_cg_residuals_grid_pdf(ch2_j.residuals ; name="X2_Krylov_cg_all")
                else
