...
function save_rank_panel_pdf(r::Int, resvec::Vector{ALSResult}, 
    ...
    lay = @layout [grid(K, 4); T{0.05h}]
    p = plot(layout=lay, size=(5000, 8000), margin=20mm, left_margin=30mm, 
             plot_title="Exact ALS Rank Panel | matrix size = $(size_B) cond = $cond_B ")

    idx(row, col) = (row-1)*4 + col
    last_ratio = zeros(K)
    ...
    headers = ["outer_tol", "inner_tol", "final_error", "total_time", "total_inner_iters", "outer_iters", "converged", "final_ratio"]
    rows = [(
...
function save_relaxation_panel_pdf(
    ...
    headers = ["inner_tol", "outer_tol", "final_err", "total_time", "total_inner_iters","outer_iters", "converged"]
    rows_tbl = [(
