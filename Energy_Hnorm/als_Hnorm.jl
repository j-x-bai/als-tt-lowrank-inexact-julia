...
function als_2d_qr(B::AbstractMatrix, rank::Int, 
    ...
    for s in 1:outer_maxiters
        ...
        if als_error[s] <= outer_tol
            println("The Hnrom solution for matrix size ", n, "x", m, " rank = ", rank, " with condition number ", cond(B), " and outer tolerance ", outer_tol, " and inner abstol", inner_abstol, " and inner reltol", inner_reltol," converged at outer iteration: ", s)
            converged_als = true
            break
        end
    end
    ...
    result = ALSResult(X1, X2, rank, outer_iters, inner_iters, total_inner_iters, als_error, converged_als, time_cg, total_time_cg, outer_tol, inner_abstol, inner_reltol, cg_last_resnorm_1, cg_last_resnorm_2, X2tX2_fnorm, bound_ratio, max_angle, upper_error_bound, last_theta_max, last_upper_error_bound, last_theor_ratio)
    return result
end
