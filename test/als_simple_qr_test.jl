    ...
    for s in 1:outer_maxiters
        ...
        Threads.@threads for core in 1:num_threads
            for j = (core-1)*blocksize1 +1 : min(core*blocksize1, n)
                # Y1[j, :] = cg(id, (X2' * B')[:, j]; abstol=inner_tol, maxiter=inner_maxiters)
        ...
        Threads.@threads for core in 1:num_threads
            for j = (core-1)*blocksize2 +1 : min(core*blocksize2, m)
                # Y2[j, :] = cg(id, (X1' * B)[:, j]; abstol=inner_tol, maxiter=inner_maxiters)
...
# B = randn(250, 250)
data = load("data/matrix/500x500/500x500_cond1811.06.jld2")
B = data["B"]
cond_B = data["cond_B"]
println("condition number: ", cond_B)
# rank = 1
rank = 125
println("condition number: ", cond(B))
B_als, als_error_his, converged_als = als_2d_qr(B, rank)
B_svdt, singular_values = svd_truncated(B, rank)

println("ALS Converged ?: ", converged_als)
# println("outer error history: ", als_error_his)

println("Rate of convergence in Power Method: ", singular_values[1]/singular_values[2])
error1 = als_error_his[end]
println("Final error from ALS: ", error1)
error2 = norm(B_svdt - B, 2)
println("Error of truncated SVD: ", error2)
error3 = norm(B_als - B_svdt, 2)
